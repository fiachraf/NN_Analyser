...
#could probably do Neuron class as an inherited class of subNeuron class
class subNeuron:
    #subNeuron is similar to the Neuron except it only holds values for that individual label
    def __init__(neuself, parent_neuron):
        neuself.parent_neuron = parent_neuron
        neuself.act_val = None
        neuself.mean = 0
        neuself.max = None
        neuself.min = None
        neuself.num_inputs = 0
        #graph item is QGraphicsItem assigned to this neuron, might need it so that the values can be updated when the analysis is finished
        # neuself.graph_item = None
        neuself.colors = {}
...
class NN_neuron:
    def __init__(neuself, neuron_index, parent_layer):
        ...
        neuself.bias = None
        neuself.act_val = None
        #labels is a dict that consists of subNeurons
        neuself.labels_dict = {}
        neuself.mean = 0
        neuself.max = None
        neuself.min = None
    ...
    #leaving this one here so you can get the activation for a single image input
    def neuron_activation(neuself, act, label):
        neuself.act_val = act
        neuself.update_vals(act, label)

    def update_vals(neuself, new_act, label):
        if neuself.num_inputs == 0:
            neuself.max = new_act
            neuself.min = new_act
        ...
        #getting details for activations differentiated by label
        if str(label) not in neuself.labels_dict:
            new_subNeuron = subNeuron(neuself)
            neuself.labels_dict[str(label)] = new_subNeuron
        neuself.labels_dict[str(label)].update_vals(new_act)
...
class NN_layer:
    def __init__(layself, layer_index, parent_net):
        ...
        layself.max = None
        layself.min = None
        layself.avg_max = None
        layself.avg_min = None
        """
    ...
    def dist_layer_activations(layself, act_array, labels_eagerTensor):
        #act array is a numpy array that has shape (len(input_list), (shape for n Dimensional layer like Conv2D), number of neurons)
        #for each input/image
        labels_array = labels_eagerTensor.numpy()
        for index_2, input_n_activation in enumerate(act_array):
            #for each neuron in layer
            for index_3, neuron_object_1 in enumerate(layself.neuron_list):
                neuron_object_1.neuron_activation(input_n_activation[..., index_3], labels_array[index_2])
                #.numpy() converts from EagerTensor to numpy array
    ...
    def layer_stats(layself):

        sum_mean = 0
        sum_max = 0
        sum_min = 0
        first_run = True
        for neuron in layself.neuron_list:
            sum_mean += neuron.mean
            sum_max += neuron.max
            sum_min += neuron.min

            if first_run == True:
                layself.max = neuron.max
                layself.min = neuron.min
                first_run = False

            elif np.mean(layself.max) < np.mean(neuron.max):
                layself.max = neuron.max

            elif np.mean(layself.min) > np.mean(neuron.min):
                layself.min = neuron.min

        layself.mean = sum_mean/len(layself.neuron_list)
        layself.avg_max = sum_max/len(layself.neuron_list)
        layself.avg_min = sum_min/len(layself.neuron_list)




class Neural_Net:
    def __init__(Netself, chosen_NN_model):
        if type(chosen_NN_model) is str:
            Netself.NN_model = models.load_model(chosen_NN_model)
        else:
            Netself.NN_model = chosen_NN_model
        Netself.num_layers = len(Netself.NN_model.layers)
        Netself.layer_list = []
        Netself.file_dir = None
        #progress through analysing the inputs, first element is number of images processed, second element is total number of images that are being processed
        Netself.progress = (0,0)
    ...
    def set_file_dir(Netself, chosen_dir):
        Netself.file_dir = chosen_dir
    ...
    def get_all_layers_activations(Netself):
        ...
        # activations = activation_model.predict(preprocess(Netself.file_dir)[1])

        preprocess_input = preprocess(Netself.file_dir)
        num_files = len(preprocess_input[1])
        Netself.progress = (0, num_files)
        #doing things in batches
        #preprocess_input[0] is the original from keras function but before my preprocesing, it has the file names, preprocess_input[1] is the finished preprocesing images
        def batch_predictions():
            for images_batch, labels_batch in preprocess_input[1]:
            ...
            #distribute the activations layer by layer
                for layer_index_1, layer_activations in enumerate(activations):
                    Netself.layer_list[layer_index_1].dist_layer_activations(layer_activations, labels_batch)

                Netself.progress = ((Netself.progress[0] + 1), Netself.progress[1])

            for layer_for_stats in Netself.layer_list:
                layer_for_stats.layer_stats()

        prediction_thread = threading.Thread(target=batch_predictions, args=())
...
"""
overall memory used by my custom Neural_Net class and all its subclasses is very small. Currently as of 5/5/22 12:22 neurons only have attributes of mean, max, min, weights, bias, last activation, num_inputs, neuron index, parent layer, layer type and they have a size of 48 bytes.
For a Neural Net with structure 16,16, 400,16, 1 and 285 input images, the total size of my Neural_Net object is only 21kB.
...
if __name__ == "__main__":
    ...
    CNN_model = models.load_model("/home/fiachra/atom_projects/meteorml/keras/meteorml_20220220_4.h5")
