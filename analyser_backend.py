...
class NN_layer:
    ...
    def dist_layer_activations(layself, act_array):
        ...
        for index_2, input_n_activation in enumerate(act_array):
            ...
            for index_3, neuron_object_1 in enumerate(layself.neuron_list):
                neuron_object_1.neuron_activation(input_n_activation[..., index_3])
    ...
    def layer_avg(layself):
        sum_1 = 0
        for neuron in layself.neuron_list:
            sum_1 += neuron.mean
        layself.mean = sum_1/len(layself.neuron_list)
...
if __name__ == "__main__":
    ...
    fuck_model = Neural_Net(CNN_model)
    fuck_model.set_file_dir("/home/fiachra/Downloads/Meteor_Files/20210131_pngs")
    print("test1")
    print(f"fuck_model: {fuck_model}")
    print(f"type(fuck_model): {type(fuck_model)}")
    print(f"fuck_model.NN_model: {fuck_model.NN_model}")
    fuck_model.get_all_layers_activations()

    print(fuck_model.layer_list[3].neuron_list[0].mean)
    print(fuck_model.layer_list[3].neuron_list[1].mean)



    #get total size of my Neural_Net class object
    # total_layers_size = 0
    # total_neurons_size = 0
    # layer_index = 0
    # for layer in fuck_model.layer_list:
    #     total_layers_size += sys.getsizeof(layer)
    #     print(f"layer_index : {layer_index}")
    #     print(f"size 1 layer: {total_layers_size}")
    #     layer_index += 1
    #     neuron_index = 0
    #     for neuron in layer.neuron_list:
    #         total_neurons_size += sys.getsizeof(neuron)
    #         print(f"size 1 neuron: {total_neurons_size}")
    #         print(f"neuron_index:{neuron_index}")
    #         neuron_index += 1
    # print(f"size of NN: {sys.getsizeof(fuck_model)}")
    # print(f"size of layers: {total_layers_size}")
    # print(f"size of neurons: {total_neurons_size}")
